...
import spconv
try:
    # spconv 2.x specific import
    from spconv.pytorch.conv import SparseConvolution
    import spconv.pytorch as spconv_core
except ImportError:
    # Fallback for older versions or mixed environments
    try:
        from spconv.conv import SparseConvolution
        spconv_core = spconv
    except:
        # Define a dummy if completely missing to prevent import error before usage
        SparseConvolution = torch.nn.Module
        spconv_core = spconv
...
# Modified Class for spconv 2.x Compatibility
class SparseConvolutionQunat(SparseConvolution, quant_nn_utils.QuantMixin):
    ...
    def __init__(
        ...
        # Directly call SparseConvolution init to avoid super() issues with MRO and old arguments
    ...
    def forward(self, input):
        ...
        if self.weight is not None:
            ...
            # Re-wrap as Parameter to satisfy nn.Module requirements, 
            # though usually strictly not needed for forward, safe for export
            self.weight = Parameter(quant_weight)

        # Call the parent forward directly
        return SparseConvolution.forward(self, input) 
 

def transfer_spconv_to_quantization(nninstance : torch.nn.Module, quantmodule):
    # Use __new__ to bypass __init__ initially
    quant_instance = quantmodule.__new__(quantmodule)
    # Copy all attributes (weights, params, config) from the original instance
    for k, val in vars(nninstance).items():
        setattr(quant_instance, k, val)
    
    # Initialize the quantizer manually since we skipped __init__
    if hasattr(quant_instance, 'init_quantizer'):
        quant_instance.init_quantizer(
            SparseConvolutionQunat.default_quant_desc_input, 
            SparseConvolutionQunat.default_quant_desc_weight
        )
...
def quantize_sparseconv_module(model):
    def replace_module(module, prefix=""):
        for name in list(module._modules.keys()):
            ...
            # Check for various forms of Sparse Convolution classes in spconv 2.x
            is_spconv = isinstance(submodule, (spconv_core.SparseConv3d, spconv_core.SubMConv3d, SparseConvolution))
...
def transfer_torch_to_quantization(nninstance : torch.nn.Module, quantmodule):
    ...
    def _init_quantizer(self):
        ...
        if isinstance(self, quant_nn_utils.QuantInputMixin):
            ...
            if hasattr(self._input_quantizer, '_calibrator') and isinstance(self._input_quantizer._calibrator, calib.HistogramCalibrator):
                self._input_quantizer._calibrator._torch_hist = True
        else:
    ...
    _init_quantizer(quant_instance)
    return quant_instance
